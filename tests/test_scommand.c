...
START_TEST (test_new_is_empty)
{
    ...
    fail_unless (scommand_length (scmd) == 0);
    scommand_destroy (scmd); scmd = NULL;
}
END_TEST
...
START_TEST (test_fifo)
{
    ...
    for (i=0; i<MAX_LENGTH; i++) {
        /* mismo string */
        fail_unless (strcmp (scommand_front (scmd),strings[i]) == 0);
        free(strings[i]);
        scommand_pop_front (scmd);
    }
    free (strings);
...
START_TEST (test_front_idempotent)
{
    ...
    scommand_push_back (scmd, strdup ("123"));
    for (i=0; i<MAX_LENGTH; i++) {
        fail_unless (strcmp (scommand_front (scmd), "123") == 0);
    }
}
END_TEST
...
START_TEST (test_front_is_not_back)
{
    ...
    fail_unless (strcmp (scommand_front (scmd), "456") != 0);
}
END_TEST
...
START_TEST (test_redir)
{
    ...
    fail_unless (strcmp (scommand_get_redir_in (scmd), scommand_get_redir_out (scmd)) == 0);
}
END_TEST
...
START_TEST (test_independent_redirs)
{
    ...
    scommand_set_redir_in (scmd, strdup ("123"));
    fail_unless (strcmp (scommand_get_redir_in (scmd), "123") == 0);
    fail_unless (scommand_get_redir_out (scmd) == NULL);

    /* Segundo: Volvemos ambos a NULL */
    scommand_set_redir_in (scmd, NULL);
    fail_unless (scommand_get_redir_in (scmd) == NULL);
    fail_unless (scommand_get_redir_out (scmd) == NULL);

    /* Tercero: sólo salida */
    scommand_set_redir_out (scmd, strdup ("456"));
    fail_unless (scommand_get_redir_in (scmd) == NULL);
    fail_unless (strcmp (scommand_get_redir_out (scmd), "456") == 0);
...
START_TEST (test_to_string)
{
    ...
    last_ocurr = str;
    for (i=0; i < MAX_LENGTH - 2; i++) {
            fail_unless(strstr(str, strings[i])>=last_ocurr);
            last_ocurr = strstr(str, strings[i]);
    }
    // Redir in
    fail_unless (strstr(str, strings[MAX_LENGTH - 2]) != NULL);
    fail_unless (strchr(str, '<') != NULL);
    fail_unless (strstr(str,strings[MAX_LENGTH - 2]) > strchr(str, '<'));
    // Redir out
    fail_unless (strstr(str, strings[MAX_LENGTH - 1]) != NULL);
    fail_unless (strchr(str, '>') != NULL);
    fail_unless (strstr(str, strings[MAX_LENGTH - 1]) > strchr(str, '>'));
    free (str);
    free(strings);
}
END_TEST
